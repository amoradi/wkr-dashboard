...
(function () {
  ...
  (function () {
    ...
    function initDashboard(options) {
      ...
      $$utilities$$fetchWeeklyReportsData(options["spreadsheetData"], parseTeamMemberScores);
    }
    ...
    function drawAvgScores() {
      ...
      document.querySelector(".Aggregate-aggregate").style.backgroundColor = $$utilities$$calculateColor([0, aggAgg])[1];
    ...
    function filterInvalidMemberFields(dimensionSet) {
      if (Number.isInteger(dimensionSet[0]) && Number.isInteger(dimensionSet[1])) {
        appendNode($$dashboard_options$$default["docFrag"], $$utilities$$doughnutChartFactory(dimensionSet, $$utilities$$calculateColor(dimensionSet)));
      }
    }
    ...
    function setMemberAvgScore(docFrag) {
      var avgScore = calcTeamMemberAvgScore(docFrag),
          ...
          color = $$utilities$$calculateColor([0, avgScore])[1];
